...
/**
 * Ensures NBA competition exists and seeds ~30 franchises from ESPN.
 * Lightweight — no roster sync (that stays on CLI / team open).
 */
export async function ensureNbaCompetition(): Promise<void> {
  ...
  if (existingCount >= 28) return;

  let entries: EspnTeamEntry[] = [];
  try {
    const response = await fetch(NBA_TEAMS_URL, {
      headers: {
        "User-Agent": "football-intelligence-platform/1.0 (nba-bootstrap)",
    ...
    console.warn("[nba-bootstrap] ESPN teams fetch failed:", error);
    return;
  }
...
export { COMPETITION_NAME as NBA_LABEL, ESPN_SLUG as NBA_ESPN_SLUG };
