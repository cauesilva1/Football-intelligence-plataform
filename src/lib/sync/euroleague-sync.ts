      ...
      try {
        ...
        if (isDbSource()) {
          await upsertPlayerMatchStat({
            ...
            goals: Math.round(parsed.points),
            assists: Math.round(parsed.assists),
            tackles: Math.round(parsed.steals),
            interceptions: Math.round(parsed.blocks),
            passesCompleted: Math.round(parsed.rebounds),
            passesAttempted: Math.round(parsed.fgAtt),
