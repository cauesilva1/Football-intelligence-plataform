...
export function normalizeRecruitmentPriorities(
  ...
  const input = (brief.priorities ?? defaults) as RecruitmentDimensionPriorities;
...
export function scoreRecruitmentFit(
  ...
  const confidence = soccerDataConfidence(player, profile);
  const limitations = [
    ...profile.limitations,
