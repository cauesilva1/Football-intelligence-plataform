...
import { checkShortlistAction } from "@/lib/actions/shortlist";
...
export async function PlayerProfileView({ playerId }: { playerId: string }) {
  const [player, isShortlisted] = await Promise.all([
    queryPlayerById(playerId),
    checkShortlistAction(playerId),
  ]);
  if (!player) notFound();

  return (
    <div className="space-y-6">
      <PlayerProfileHeader player={player} isShortlisted={isShortlisted} />
