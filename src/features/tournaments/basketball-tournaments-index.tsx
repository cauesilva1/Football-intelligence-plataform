...
import { ArrowRight, CircleDot, GraduationCap } from "lucide-react";
import { BASKETBALL_COMPETITIONS } from "@/lib/tournaments/basketball-competitions";
...
export function BasketballTournamentsIndex() {
  return (
    <div className="space-y-8">
      <div className="sport-hero overflow-hidden rounded-2xl border border-primary/20 p-5 shadow-panel md:p-8">
        <p className="text-2xs font-semibold uppercase tracking-[0.2em] text-primary">Leagues</p>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {BASKETBALL_COMPETITIONS.map((competition) => {
            const Icon = ICONS[competition.slug as keyof typeof ICONS] ?? CircleDot;
            return (
              <Link
